...
use TempFull, clear
include "02_JapanMTUS/1996/AuxVars1996.do"
...
* Note: In the 1996 survey, there is no variable on the diary order
...
replace agekid2 = CAgeNatYg  if  inrange(CAgeNatYg7Grp, 0, 6)

* Note: 
*	In STULA1996, age of youngest child is measured only for couples with children.
*	Age of children with a single parent is missing.
...
replace ownhome =  2 if inlist(QHouseType, 3, 4, 5, 6, 7, 8)
...
********************************************************************************
gen     empstat =  .
replace empstat =  1 if        QEmpStatY == 1
replace empstat =  2 if inlist(QEmpStatY, 2, 3)
replace empstat =  3 if inlist(QEmpStatY, 4, 5, 6)
replace empstat =  4 if        QEmpStatX == .
replace empstat = -9 if        QEmpStatY == 9	
...
********************************************************************************
gen     emp = .
replace emp = 0 if QEmpStatX == .
replace emp = 1 if QEmpStatX != .
...
********************************************************************************
gen     unemp = .
replace unemp = 1 if QEmpStatX != .
replace unemp = 0 if QEmpStatX == .
